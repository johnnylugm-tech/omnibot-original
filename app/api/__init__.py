...
from .helpers import get_active_conversation, get_emotion_tracker, get_or_create_user
...
async def process_webhook_message(
    ...
    with tracer.start_as_current_span(f"process_{platform}_message") as span:
        ...
        state = dst_manager.process_turn(int(conv.id), intent, {"content": clean_text})
        ...
        if pii_masking.should_escalate(clean_text) or tracker.should_escalate():
            ...
            try:
                ...
                result = await knowledge_layer.query(
                    processed_text, user_context={"state": state.current_state.value}
                )
...
async def telegram_webhook(
    ...
    if x_telegram_bot_api_secret_token and TELEGRAM_BOT_TOKEN:
        if not verify_signature(
            "telegram", body, x_telegram_bot_api_secret_token, TELEGRAM_BOT_TOKEN
        ):
