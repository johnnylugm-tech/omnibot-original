...
async def test_id_48_01_privilege_escalation_editor_to_delete():
    ...
    # We will simulate an editor trying to use an admin-like string if validation is weak
    response = client.delete("/api/v1/knowledge/1", headers={"Authorization": "Bearer admin_spoof"})
...
async def test_id_48_02_token_tampering_malformed_json():
    ...
    response = client.get("/api/v1/knowledge?q=test", headers={"Authorization": "Bearer not.a.jwt"})
