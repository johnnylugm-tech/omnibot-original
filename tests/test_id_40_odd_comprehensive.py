...
async def test_id_40_09_security_block_rate(mock_db):
    ...
    mock_result = MagicMock()
    mock_result.fetchall.return_value = [MagicMock(_mapping={"block_rate": 2.5})]
    mock_db.execute.return_value = mock_result
...
async def test_id_40_11_pii_masking_rate(mock_db):
    ...
    mock_result = MagicMock()
    mock_result.fetchall.return_value = [MagicMock(_mapping={"masking_rate": 12.0})]
    mock_db.execute.return_value = mock_result
...
async def test_id_40_12_rbac_denial_audit(mock_db):
    ...
    mock_result.fetchall.return_value = [
        MagicMock(
            _mapping={"role": "editor", "resource": "knowledge", "denial_count": 5}
        )
    ]
    mock_db.execute.return_value = mock_result
