...
@pytest.mark.skipif(
    not HAS_HYBRID, reason="app.services.hybrid_knowledge not yet implemented"
)
@pytest.mark.asyncio
async def test_hybrid_layer_rule_match_confidence_above_0_9():
    ...
    When confidence > 0.9, the method should short-circuit RRF and return
    the rule result.
    RAG must NOT be called when rule confidence is already high enough.
    """
    HybridKnowledgeLayer = hybrid_knowledge.HybridKnowledgeLayer  # noqa: N806
    KnowledgeResult = hybrid_knowledge.KnowledgeResult  # noqa: N806
    mock_db = AsyncMock()

    with patch.object(
        HybridKnowledgeLayer, "_rule_search", new_callable=AsyncMock
    ) as mock_rule:
        # Layer 1 returns high-confidence rule match (0.95 > 0.9)
        mock_rule.return_value = [
            KnowledgeResult(
                id=1, content="high confidence rule doc", confidence=0.95, source="rule"
            )
        ]

        with patch.object(
            HybridKnowledgeLayer, "_rag_search", new_callable=AsyncMock
        ) as mock_rag:
...
async def test_hybrid_layer_rule_match_confidence_below_0_9_triggers_rrf():
    ...
    with patch.object(
        ...
        mock_rule.return_value = [
            KnowledgeResult(
                id=1, content="borderline rule doc", confidence=0.85, source="rule"
            )
        ]
        ...
        with patch.object(
            ...
            mock_rag.return_value = [
                KnowledgeResult(
                    id=2,
                    content="rag doc with higher fused rank",
                    confidence=0.88,
                    source="rag",
                )
            ]
...
def test_llm_layer3_base_prompt_is_reproducible():
    ...
    if prompt_config_path:
        assert os.path.isfile(prompt_config_path), (
            f"LLM_LAYER3_PROMPT_CONFIG points to non-existent file: {prompt_config_path}"  # noqa: E501
        )
        ...
        # A proper template should have at least one placeholder like {query} or {context}  # noqa: E501
        assert "{" in content, (
            f"Prompt template should contain placeholders like {{query}}, got: {content[:100]}"  # noqa: E501
        )
