...
def test_health_check_status_unhealthy_when_service_down(client, mock_db):
    """When both Postgres and Redis are down, health returns status=degraded (not healthy)."""  # noqa: E501
...
def test_rate_limit_429(client):
    with patch("app.security.RateLimiter.check", new_callable=AsyncMock) as mock_check:
        ...
        response = client.post(
            ...
            json={"message": {"text": "hi", "from": {"id": 1}}},
        )
...
def test_telegram_webhook_normal(mock_get_conv, mock_get_user, mock_process, client):
    ...
    payload = {"message": {"from": {"id": 12345}, "text": "你好，我想詢問課程資訊"}}
...
def test_knowledge_crud_rbac(client, mock_db):
    ...
    # Create
    response = client.post(
        "/api/v1/knowledge", json={"question": "new", "answer": "ans"}, headers=headers
    )
    assert response.status_code == 200
    ...
    mock_k = KnowledgeBase(
        id=kid, question="new", answer="ans", is_active=True, version=1
    )
    mock_db.execute.return_value.scalar_one_or_none.return_value = mock_k

    # Update
    response = client.put(
        f"/api/v1/knowledge/{kid}", json={"answer": "updated"}, headers=headers
    )
    ...
    response = client.post(
        ...
        json={"items": [{"question": "1", "answer": "1"}]},
        headers=headers,
    )
    assert response.status_code == 200
...
def test_conversations_list_rbac(client):
    response = client.get(
        ...
        headers={
            "Authorization": "Bearer eyJyb2xlIjogImFkbWluIn0.78f2e30e15e2d24f56e94b18a4b8fb611313993b48a78d4bca5895c6764f1f5a"  # noqa: E501
        },
    )
    assert response.status_code == 200
    ...
    response = client.get(
        "/api/v1/conversations",
        headers={
            "Authorization": "Bearer eyJyb2xlIjogImFnZW50In0.c7885d0c7f0dfc6b944c9633ebd12c237c609258bd643938e9366204c6c95dc6"  # noqa: E501
        },
    )
    ...
    response = client.get(
        "/api/v1/conversations",
        headers={
            "Authorization": "Bearer eyJyb2xlIjogInVua25vd24ifQ.ee34e1fe9156751d5f00c5fd77bf11ff4e48a4e6e1d5d017b230d82f820cd0c4"  # noqa: E501
        },
    )
    assert response.status_code == 403
